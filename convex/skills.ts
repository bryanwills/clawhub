...
function isClawScanMaliciousVersion(
  version: Pick<Doc<"skillVersions">, "clawScanVerdict" | "llmAnalysis">,
) {
  return (
    normalizeClawScanVerdict(
...
async function patchStructuredModerationFromVersion(
  ...
  const shouldPersistClawScanMalwareBlock =
    patch.moderationVerdict === "malicious" && isClawScanMaliciousVersion(version);
...
async function scheduleClawScanAutobanForMalware(
  ctx: MutationCtx,
  skill: Doc<"skills">,
  version: Pick<Doc<"skillVersions">, "clawScanVerdict" | "llmAnalysis" | "sha256hash">,
  patch: SkillModerationPatch,
) {
  if (
    patch.moderationVerdict === "malicious" &&
    skill.ownerUserId &&
    isClawScanMaliciousVersion(version)
  ) {
  ...
  handler: async (ctx, args) => {
    ...
    if (!skill) {
      ...
      const newSkillFlags = Array.from(
        new Set([...(derivedFlags ?? []), ...(initialScannerSnapshot.moderationFlags ?? [])]),
      );
      const skillId = await ctx.db.insert("skills", {
        ...
        moderationReason,
        moderationNotes,
        moderationVerdict: initialScannerSnapshot.verdict === "malicious" ? "malicious" : "clean",
